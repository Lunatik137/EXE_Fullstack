...
const saveManualMealPick = async (req, res) => {
  try {
    ...
    // Validate rice/non-rice constraints for lunch/dinner
    if ((mealType === "lunch" || mealType === "dinner") && validRecipeIds.length > 0) {
      let riceCount = 0;
      let nonRiceCount = 0;
      
      for (const recipeId of validRecipeIds) {
        const recipe = await recipeModel.findById(recipeId).select("name");
        if (isWhiteRiceRecipeName(recipe.name)) {
          riceCount++;
        } else {
          nonRiceCount++;
        }
      ...
      if (nonRiceCount > 3) {
        return res.json({ success: false, message: "Tối đa 3 món khác nhau cho bữa này" });
      }
      if (riceCount > 1) {
        return res.json({ success: false, message: "Tối đa 1 cơm trắng cho bữa này" });
      }
    }
